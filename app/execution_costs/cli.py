...
from app.historical_replay.data import load_manifest, immutable_json, verify_seal, HistoricalError, END, START
from app.historical_replay.models import ExecutionModel
from .configuration import configuration, run_manifest, INSTANCE
from .storage import QuantifiedStore, hget, hput
from app.historical_replay.index import build_index, verify_index
from .engine import QuantifiedPaper
...
def main(argv=None):
    p=argparse.ArgumentParser(description='8D quantity-consistent historical OFFLINE replay; no account or order network')
    p.add_argument('command',choices=('index','freeze','init','run','recover','report','attribute'))
    p.add_argument('--workspace',default='.')
    p.add_argument('--dataset',required=True)
    p.add_argument('--run-id',default='august-8d-baseline-v1')
    ...
    p.add_argument('--output',help='New JSON result file; refuses overwrite and unsafe paths')
    ...
    try:
        if args.command=='index':
        ...
        elif args.command in ('init','attribute'):
            ...
            if args.command=='attribute':
                if not args.source_run:raise HistoricalError('EXPLICIT_READ_ONLY_SOURCE_RUN_REQUIRED')
                if args.limit_candidates is not None and args.limit_candidates<=0:raise HistoricalError('POSITIVE_CANDIDATE_LIMIT_REQUIRED')
                from .attribution import run_attribution
                output=run_attribution(args.workspace,args.source_run,bundle,settings,run,model,limit=args.limit_candidates)
            else:
                store=QuantifiedStore.initialize(args.workspace,args.run_id,settings,bundle,load_manifest(args.dataset),run)
                paper=QuantifiedPaper(store);paper.recover();output={'initialized':True,'run_digest':run['content_digest'],'live_allowed':False}
        else:
            store=QuantifiedStore(args.workspace,args.run_id,INSTANCE);paper=QuantifiedPaper(store)
        ...
        if args.output: immutable_json(check_owned(args.output),output)
