...
def result(paper):
    with localcontext(Context(prec=50)),paper.store.transaction() as db:
        ...
        return dict(version='quantified-historical-report/v1',scope=run['scope'],experiment_id=run['experiment_id'],run_digest=run['content_digest'],
            ...
            funding_budget_breaches=[v for k,v in hrows(db,'history_meta') if k.startswith('funding-limit:')],
