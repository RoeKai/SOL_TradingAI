...
from .models import HistoricalSettings
from .evidence import VERIFIER
from .funding import RISK_SOURCE
from .data import HistoricalError, seal, START, END, WARMUP

INSTANCE='historical-paper-august-2026'
...
def configuration(workspace,model):
    ...
    settings=HistoricalSettings(instance_id=INSTANCE,initial_balance='500',initial_time=WARMUP//1000,
...
def run_manifest(*,experiment_id,code_commit,code_digest,dataset,bundle,settings,model,kind='baseline',end_ms=END):
    ...
    return seal(dict(version='historical-run/v1',experiment_id=experiment_id,kind=kind,code_commit=code_commit,
        ...
        exit_plan_version='historical-exit-plan/v1',scenario_version='historical-conditional-scenarios/v1',
        ...
        funding_budget_formula='500 USDT frozen notional ceiling * 0.001 * 2 settlement allowances per <=3600s plan; no future-rate access',
        ...
        thresholds='unchanged accepted Stage 5; separate 8C verifier and funding-aware ledger identity',
        performance_selection='fixed dates and parameters before run; no strategy optimization',
