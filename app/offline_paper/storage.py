...
def get(db,table,key):
    if table not in TABLES: raise OfflineError('Unknown table')
...
class Store:
    ...
    def __init__(self,workspace,run_id,instance_id):
        ...
        self.root=self.paths.file('offline-runs/'+run_id)
        self.path=self.paths.file('offline-runs/'+run_id+'/ledger.sqlite3')
    ...
    def create(cls,workspace,run_id,settings,bundle):
        ...
        store.paths.directory('offline-runs/'+run_id)
        ...
        try:
            ...
            for table in TABLES: db.execute('CREATE TABLE '+table+' (id TEXT PRIMARY KEY, payload TEXT NOT NULL)')
    ...
    def connect(self):
        ...
        try:
            ...
            if tables!=set(TABLES): raise OfflineError('Database schema mismatch')
            identity=get(db,'identity','identity')
            settings=RunSettings.model_validate(identity['settings'])
    ...
    def settings(self,db):
        return RunSettings.model_validate(get(db,'identity','identity')['settings'])
