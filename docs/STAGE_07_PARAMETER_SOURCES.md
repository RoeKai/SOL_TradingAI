...
### 字段路径与原始 YAML 键（R1 明确语法边界）

表格中的 `risk.max_loss_per_trade` 等是内部语义路径，不是可直接填写的 YAML 字面键。原文件只支持嵌套字段，例如 `risk: {max_loss_per_trade: 5}`；每个原始键必须是非空、不含点号的字符串。点号键在原始映射构建时即被拒绝，不能进入 flatten、别名合并、默认值或来源字典。同义字段别名仅限原先定义的嵌套 `drop_pct` / `btc_max_drop_pct`，仍按负百分数转换后检查一致性；单独填写时来源指向别名，同时填写且等价时保留两个来源。未增加点号覆盖语法、隐式取值优先级或来源丢弃规则。字符串值和生成的来源路径中的点号仍合法。
