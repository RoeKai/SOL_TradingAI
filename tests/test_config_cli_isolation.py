...
def test_pure_modules_exact_dependencies_and_no_integration_anywhere_else():
    ...
    for path in [ROOT/'main.py',*(ROOT/'app').rglob('*.py')]:
        ...
        if path.relative_to(ROOT).as_posix() in {'app/offline_paper/storage.py',
            ...
            'app/execution_costs/plans.py','app/execution_costs/attribution.py',
            'app/scenario_diagnostics/source.py','app/scenario_diagnostics/proofs.py',
            'app/scenario_diagnostics/costs.py'}: continue
